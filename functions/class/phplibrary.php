...
class VideoBlogger_PHP{
    ...
    */
     ...
     * @param array $pro
     */
    public function count_csvfilerows($csvfile_name)
    {    
        return count(file(WTG_VB_CONTENTFOLDER_DIR . $csvfile_name)) - 1;
    }    
    /**
    ...
    */
    function alphanumeric($string,$minimum_length = 1,$maximum_length = 99)
    { 
    ...
    */
    function strto_timeformat($date_string,$format = 'UK')
    {
        if($format == 'US'){
    ...
    */
    function prepend_exists($haystack,$needle)
    {
    ...
    */
    ...
    /**
    * compares option to stored setting and echos selected if match found

    * @return nothing  
    */             
    public function echoselected( $value,$argument){
        if( $value == $argument ){echo 'selected="selected"';}
    } 
    /**
    ...
    */
    public function arrayinfo_set($array,$line,$function,$file){
        ...
        $array['arrayinfo']['version'] = $wtgvb_currentversion;
        $array['arrayinfo']['line'] = $line;
        $array['arrayinfo']['function'] = $function;
        $array['arrayinfo']['file'] = $file;
        $array['arrayinfo']['date'] = VideoBlogger_WP::datewp();
        return $array;   
    }  
    /**
    ...
    * 2. Faster than wtgvb_count_extension_in_directory() because it returns sooner, important for directory with a lot of files
    * 
    * @returns boolean
    */
    ...
    */
    ...
    {
        if(VideoBlogger_PHP::is_image_url($value)){
            return 'image';
        }
        
        // so its not an image url, is it any type of url? either for using as link or PHP functionality
        if(VideoBlogger_PHP::is_url($value)){
            return 'url';    
        }
        
        // check if decimal before checking if a plain number or money
        if(VideoBlogger_PHP::is_decimalnumber($value)){
            return 'decimal';
        }

        // is money value
        if(VideoBlogger_PHP::is_string_money($value)){
            return 'money'; 
        }
        ...
        // is email address
        if(VideoBlogger_PHP::is_valid_emailaddress($value)){
            return 'email';
        }                  
    ...
    */
    public function months_time_range($month,$year,$return = 'array',$format = 'time')
    {
        ...
        if($format == 'date')
        {
            $array['start'] = VideoBlogger_WP::datewp(0,$array['start']);
        }
        ...
        if($format == 'date')
        {    
            $array['end'] = VideoBlogger_WP::datewp(0,$array['end']);   
        }
